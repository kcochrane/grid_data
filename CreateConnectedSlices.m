...
function [slices,xstruct] = CreateConnectedSlices(M,x_topo,y_topo,V,im,n)
load inferno.mat 
...
for k = 1:n-1
    ...
    pold = p;
    [pxt,pyt] = ginput(1); %Asks for click input for point k+1
    p = [pxt pyt];
    p = round(p);
    plot(p(1),p(2),'r.','MarkerSize',15)
    ...
    %dist = sqrt((y_topo(p(2))-y_topo(pold(2))).^2+(x_topo(p(1))-x_topo(pold(1))).^2);
    dist = sqrt((y_topo(p(1))-y_topo(pold(1))).^2+(x_topo(p(2))-x_topo(pold(2))).^2);
